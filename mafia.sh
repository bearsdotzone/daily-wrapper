#!/bin/sh
curl -fLgo kolmafia.jar $(curl -fg 'https://api.github.com/repos/kolmafia/kolmafia/releases/latest' | jq -r '.assets[] | select(.browser_download_url | contains(".jar")).browser_download_url')
